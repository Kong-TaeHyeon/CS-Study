...
**서버 푸시**  
또한, HTTP/2.0에서는 클라이언트의 요청없이 이와 관련된 파일들을 먼저 제공할 수 있는 서버 푸시 기능이 존재합니다.

- Ex. HTML 파일 요청 -> 이와 관련된 CSS, JS 제공.

---

#### HTTPS

HTTP/2는 HTTPS 위에서 동작합니다. HTTPS는 애플리케이션 계층과 전송 계층 사이에 신뢰 계층인 SSL/TLS 계층을 넣은 **신뢰할 수 있는 HTTP 요청**을 말합니다. 이를 통해 '통신을 암호화'합니다.

이러한 통신을 위한 암호화 중에서 '키'를 사용한 암호화에는 크게 두 가지 암호화가 존재합니다.

1. 대칭키 암호화
   대칭키를 통해 암호화를 하는 경우, 동일한 키를 통해 암호화와 복호화가 가능하게 됩니다.

2. 공개키 / 비밀키 암호화
   이 경우에는 공개키를 통해 암호화한 데이터를 비밀키로만 복호화하를 할 수 있게 됩니다.

- 비대칭키 암호화(예: RSA)는 암호화·복호화 과정이 매우 복잡하고, CPU 리소스를 많이 소모한다는 단점이 존재합니다.

따라서, 대량의 트래픽이 발생하는 인터넷 환경에서는 공개키/비밀키 암호화를 활용하여 서버와 클라이언트 간의 대칭키를 생성하는 방식을 활용하게 됩니다. (SSL/TLS)
...
<image src="./image/ssl-tls.png"/>

즉, 그림과 같이 서버의 공개키-비밀키 알고리즘을 사용하여 대칭키 생성의 재료를 공유하고 서로 동일한 대칭키를 통해 데이터를 암호화하여 전송하게 됩니다.

---

#### HTTP/3

HTTP/3은 이전의 HTTP와는 달리 UDP 기반으로 작동합니다.

이는 TCP를 사용하던 이전의 핸드쉐이크 과정을 거치지 않고 동작하기 때문에 **초기 연결 설정 시간이 감소**하게 됩니다.
